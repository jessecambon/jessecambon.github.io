...
We can now see there are two available results for each address. Note that the US Census service API does not have a `limit` argument in its API, but we can still use the `limit` parameter in `geocode()` or `geo()`. Refer to the [api_parameter_reference](https://jessecambon.github.io/tidygeocoder/reference/api_parameter_reference.html) documentation for more details.
